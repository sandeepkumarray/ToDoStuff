...
namespace ToDoStuff
{
    ...
    {
        public void CreateExcelFile(DataSet data, string OutPutFilePath)
        {
            var datetime = DateTime.Now.ToString().Replace("/", "_").Replace(":", "_");

            string fileFullname = Path.Combine(OutPutFilePath);

            if (File.Exists(fileFullname))
            {
                fileFullname = Path.Combine(OutPutFilePath);
            }

            using (SpreadsheetDocument package = SpreadsheetDocument.Create(fileFullname, SpreadsheetDocumentType.Workbook))
            {
                GenerateSheetdataForDetails(package, data);
            }
        }

        private void GenerateSheetdataForDetails(SpreadsheetDocument document, DataSet data)
        {
            foreach (DataTable table in data.Tables)
            {
                ...
                sheetData1.Append(CreateHeaderRowForExcel(Columns));

                foreach (DataRow dr in table.Rows)
                {
                    Row partsRows = GenerateRowForChildPartDetail(dr, Columns);
                    sheetData1.Append(partsRows);

                }

                WorkbookPart workbookPart1 = document.AddWorkbookPart();
                GenerateWorkbookPartContent(workbookPart1, table.TableName);
        ...
        private Row CreateHeaderRowForExcel(List<string> Columns)
        {
            Row workRow = new Row();
        ...
        private Row GenerateRowForChildPartDetail(DataRow datarow, List<string> Columns)
        {
            Row tRow = new Row();
            foreach (string column in Columns)
            {
                tRow.Append(CreateCell(datarow[column].ToString()));
            }
            return tRow;
        }
