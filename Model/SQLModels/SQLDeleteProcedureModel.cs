...
{
    public class SQLDeleteProcedureModel : BaseSQLModel
    {
        public SQLDeleteProcedureModel()
        {
        }

        public SQLDeleteProcedureModel(string procedure_name)
        {
            this.procedure_name = procedure_name;
        }

        public SQLDeleteProcedureModel(string procedure_name, string procedure_body, string input_params, string output_params)
        {
            this.procedure_name = procedure_name;
        ...
        public override BaseSQLModel Initialize()
        {
            ...
            this.procedure_name = "delete_" + this.TableName;

            string updateColumns = string.Join(",", from n in ClassProperties select n.PropName + " = p_" + n.PropName);

            var CommandText = "DELETE FROM `" + TableName.ToCamelCaseWithSeparator() + "`" + where_condition + ";";
